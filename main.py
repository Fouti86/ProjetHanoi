...
def main():
    ...
    # === 2. Initialisation caméra + détection
    print("Initialisation de la caméra...")
    processor = CameraProcessor()
    ...
    if frame is not None:
        ...
        print("Erreur : aucune image détectée")
        return
    ...
    # === 3. Calcul de l’algorithme
    print("Calcul des déplacements...")
    algo = HanoiIterative(validated_count)

    # === 4. Lancement de la simulation graphique
    app = QApplication(sys.argv)
    simulation = SimulationMoves(algo)
    simulation.show()

    # === 5. Exécution du robot (en parallèle pour ne pas bloquer l'interface)
    def executer_robot():
        for coup, origine, destination, palets_origin_before, palets_destination_before in algo.get_move_matrix():
            print(f"Exécution du déplacement {coup}: {origine} -> {destination}")
            robot.realiser_deplacement(origine, destination, palets_origin_before, palets_destination_before)
    ...
    robot_thread = threading.Thread(target=executer_robot)
    robot_thread.start()

    # === 6. Lancement de l’interface Qt (boucle événementielle)
    sys.exit(app.exec())
